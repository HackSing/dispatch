...
- **单点执行**:一个 agent 走两阶段协议(先写 plan.md 方案,经用户确认闸放行后再执行并写 result.json),完成判定不达标即失败,fail_reason 精确到环节。
- **方案确认闸**:方案产出后任务暂停(`awaiting_confirm`)并通知用户;详情页可查看方案、与主智能体多轮讨论修订、一键确认放行或放弃——确认后才产生实际代码改动,单点与工作流两种模式一致。
